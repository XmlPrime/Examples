...
namespace XmlPrime.Samples.BuiltInModules
{
    /// <summary>
    /// A simple example demonstrating use of built-in modules in XQuery.
    /// </summary>
	...
	internal class Program
	{
		...
		private static void PerformQuery(string inputPath, string queryPath, string outputPath)
		{
			...
			// First, we create an XQuerySettings instance. This describes all the settings used for compilation. 
			...
			var querySettings = new XQuerySettings
			                    	{
                                    XQueryVersion = XQueryVersion.XQuery10,
			                    		ContextItemType = XdmType.Node,
			                    		EnabledFeatures = XQueryOptionalFeatures.AllFeatures ^ XQueryOptionalFeatures.UpdateFeature
			                    	};

            // We want to make XPath and XQuery Functions and Operators 3.0 available.
            querySettings.ImportModule(XdmModule.XQuery30Functions);
            ...
            query.Serialize(outputPath, contextDocument.CreateNavigator());
		}
